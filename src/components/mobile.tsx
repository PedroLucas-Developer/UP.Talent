        ...
        <div className='flex w-full relative ml-auto px-8 pt-5 flex-row'>
          <div className='justify-center flex w-16 h-16 bg-[#61c3ac] ml-auto my-auto rounded-full' onClick={toggleMenu}>  
            <button aria-label="Home" className="flex z-50 flex-col relative text-3xl my-auto cursor-pointer">
              <div className={`w-9 h-1 bg-[#000000] my-1 rounded-lg transition-transform duration-300 transform origin-center ${ isOpen ? 'transform rotate-45 translate-y-2 bg-[#ffffff]' : ''}`}></div>
              <div className={`w-9 h-1 bg-[#000000] my-1 rounded-lg transition-opacity duration-300 ${ isOpen ? 'opacity-0 bg-[#E8336A]' : ''}`}></div>
              <div className={`w-9 h-1 bg-[#000000] my-1 rounded-lg transition-transform duration-300 transform origin-center ${ isOpen ? 'transform -rotate-45 -translate-y-4 translate-x-0 bg-[#ffffff]' : ''}`}></div>
            </button>
          </div>
          <AnimatePresence>
            {isOpen && (
              <motion.div initial={{ opacity: 0, x: 100, }} animate={{ opacity: 1, x: 25, }} exit={{ opacity: 0, x: 100,}} transition={{ duration: 0.5 }} style={{ zIndex: 0 }} className="absolute z-50 flex justify-center top-[-2rem] right-0 w-11/12 max-w-[500px] h-screen bg-black max-sm:right-0">
